...
TNode<Object> FlagGetter_335(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_receiver, JSRegExp::Flag p_flag, int31_t p_counter, const char* p_methodName) {
  ...
  if (block0.is_used()) {
    ...
    USE(tmp2);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp2 = Cast8JSRegExp_1490(state_, TNode<Context>{tmp0}, TNode<Object>{tmp1}, &label0);
...
USE(parameter1);
  ...
  if (block0.is_used()) {
    ...
    USE(tmp2);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp2 = Cast14ATFastJSRegExp_1493(state_, TNode<Context>{tmp0}, TNode<Object>{tmp1}, &label0);
