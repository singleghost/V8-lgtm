...
TF_BUILTIN(CallIteratorWithFeedback, CodeStubAssembler) {
  ...
  if (block0.is_used()) {
    ...
    USE(tmp7);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp7 = Cast71UT19ATCallableApiObject17ATCallableJSProxy15JSBoundFunction10JSFunction_1434(state_, TNode<Context>{tmp0}, TNode<Object>{tmp2}, &label0);
  ...
  if (block1.is_used()) {
    ...
    USE(tmp38);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp38 = Cast10JSReceiver_1433(state_, TNode<Context>{tmp30}, TNode<Object>{tmp37}, &label0);
