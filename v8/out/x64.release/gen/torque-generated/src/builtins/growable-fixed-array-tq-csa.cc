...
TorqueStructGrowableFixedArray NewGrowableFixedArray_307(compiler::CodeAssemblerState* state_) {
  ...
  if (block0.is_used()) {
    ...
    USE(tmp0);
    tmp0 = kEmptyFixedArray_214(state_);
    TNode<IntPtrT> tmp1;
    USE(tmp1);
    tmp1 = FromConstexpr8ATintptr17ATconstexpr_int31_150(state_, 0);
    TNode<IntPtrT> tmp2;
    USE(tmp2);
  ...
  return TorqueStructGrowableFixedArray{TNode<FixedArray>{tmp6}, TNode<IntPtrT>{tmp7}, TNode<IntPtrT>{tmp8}};
}
