...
TF_BUILTIN(ReflectSetPrototypeOf, CodeStubAssembler) {
  ...
  if (block1.is_used()) {
    ...
    USE(tmp20);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp20 = Cast21UT6ATNull10JSReceiver_1494(state_, TNode<Context>{tmp16}, TNode<Object>{tmp18}, &label0);
...
TF_BUILTIN(ReflectGet, CodeStubAssembler) {
  ...
  if (block3.is_used()) {
    ...
    USE(tmp31);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp31 = Cast10JSReceiver_1433(state_, TNode<Context>{tmp28}, TNode<Object>{tmp30}, &label0);
