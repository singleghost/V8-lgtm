...
TF_BUILTIN(ProxyDeleteProperty, CodeStubAssembler) {
  ...
  USE(parameter2);
  TNode<Smi> parameter3 = UncheckedCast<Smi>(Parameter(Descriptor::kLanguageMode));
  USE(parameter3);
  ...
  if (block5.is_used()) {
    ...
    USE(tmp20);
    tmp20 = UnsafeCast10JSReceiver_1486(state_, TNode<Context>{tmp12}, TNode<Object>{tmp19});
    ca_.SetSourcePosition("../../src/builtins/proxy-delete-property.tq", 35);
    TNode<JSReceiver> tmp21;
    USE(tmp21);
    compiler::CodeAssemblerLabel label0(&ca_);
    tmp21 = GetMethod_247(state_, TNode<Context>{tmp12}, TNode<Object>{tmp17}, "deleteProperty", &label0);
  ...
  if (block7.is_used()) {
    ...
    TNode<Object> tmp50;
    USE(tmp50);
    tmp50 = CodeStubAssembler(state_).Call(TNode<Context>{tmp43}, TNode<JSReceiver>{tmp49}, TNode<Object>{tmp47}, TNode<Object>{tmp48}, TNode<Object>{tmp45});
    ca_.SetSourcePosition("../../src/builtins/proxy-delete-property.tq", 43);
    TNode<BoolT> tmp51;
    USE(tmp51);
    tmp51 = ToBoolean_242(state_, TNode<Object>{tmp50});
    TNode<BoolT> tmp52;
    USE(tmp52);
  ...
  if (block11.is_used()) {
    ...
    TNode<BoolT> tmp62;
    USE(tmp62);
    tmp62 = CodeStubAssembler(state_).SmiEqual(TNode<Smi>{tmp56}, TNode<Smi>{tmp61});
  ...
  if (block13.is_used()) {
    ...
    TNode<Object> tmp71;
    USE(tmp71);
    tmp71 = FromConstexpr20UT5ATSmi10HeapObject18ATconstexpr_string_170(state_, "deleteProperty");
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{tmp63}, MessageTemplate::kProxyTrapReturnedFalsishFor, TNode<Object>{tmp71}, TNode<Object>{tmp65});
  }
  ...
  if (block12.is_used()) {
    ...
    ProxiesCodeStubAssembler(state_).CheckDeleteTrapResult(TNode<Context>{tmp81}, TNode<JSReceiver>{tmp86}, TNode<JSProxy>{tmp82}, TNode<Name>{tmp83});
  ...
  if (block2.is_used()) {
    ...
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{tmp96}, MessageTemplate::kProxyRevoked, "deleteProperty");
  }
}
