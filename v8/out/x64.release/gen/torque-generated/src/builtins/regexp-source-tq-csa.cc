...
USE(parameter1);
  ...
  if (block3.is_used()) {
    ...
    TNode<IntPtrT> tmp12 = ca_.IntPtrConstant(JSRegExp::kSourceOffset);
    USE(tmp12);
    TNode<HeapObject>tmp13 = CodeStubAssembler(state_).LoadReference<HeapObject>(CodeStubAssembler::Reference{tmp11, tmp12});
  ...
  if (block5.is_used()) {
    ...
    CodeStubAssembler(state_).ThrowTypeError(TNode<Context>{tmp22}, MessageTemplate::kRegExpNonRegExp, "RegExp.prototype.source");
  }
  ...
  if (block6.is_used()) {
    ...
    TNode<Object> tmp28;
    USE(tmp28);
    tmp28 = FromConstexpr90UT8ATBigInt7ATFalse6ATNull5ATSmi6ATTrue11ATUndefined10HeapNumber10JSReceiver6String6Symbol18ATconstexpr_string_171(state_, "(?:)");
