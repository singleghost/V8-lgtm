...
TNode<FixedArray> GetCoverageInfo_308(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<JSFunction> p_function, compiler::CodeAssemblerLabel* label_IfNoCoverageInfo) {
  ...
  if (block8.is_used()) {
    ...
    USE(tmp39);
    tmp39 = UnsafeCast14ATCoverageInfo_1491(state_, TNode<Context>{tmp33}, TNode<Object>{tmp38});
...
void IncrementBlockCount_311(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<FixedArray> p_coverageInfo, TNode<Smi> p_slot) {
  ...
  if (block13.is_used()) {
    ...
    USE(tmp140);
    std::tie(tmp139, tmp140) = UnsafeNewReference20UT5ATSmi10HeapObject_1440(state_, TNode<HeapObject>{tmp131}, TNode<IntPtrT>{tmp138}).Flatten();
  ...
  if (block9.is_used()) {
    ...
    TNode<Smi> tmp247;
    USE(tmp247);
    tmp247 = UnsafeCast5ATSmi_1439(state_, TNode<Context>{tmp227}, TNode<Object>{tmp246});
    TNode<Smi> tmp248;
    USE(tmp248);
...
TNode<FixedArray> UnsafeCast14ATCoverageInfo_1491(compiler::CodeAssemblerState* state_, TNode<Context> p_context, TNode<Object> p_o) {
  compiler::CodeAssembler ca_(state_);
